...
def random_masking(x, pos, info, h, remain_num):
        """
        ...
        return poly_reserve, pos_reserve, h_reserve, pos_tar
...
def main(args):
    ...
    # define the model
    if args.add_maxpool:
        model = MAGECityPosition_Maxpool(drop_ratio = args.drop_ratio, num_heads=args.num_heads, device = args.device,
        ...
        if args.patchify:
            model = MAGECityPosition_Minlen(drop_ratio = args.drop_ratio, num_heads=args.num_heads, device = args.device,
                                depth=args.trans_deep, embed_dim=args.embed_dim, decoder_embed_dim = args.decoder_embed_dim,
                                decoder_depth=args.trans_deep_decoder, decoder_num_heads=args.num_heads, pos_weight = args.pos_weight)
        else:
            model = MAGECityPosition(drop_ratio = args.drop_ratio, num_heads=args.num_heads, device = args.device,
                                depth=args.trans_deep, embed_dim=args.embed_dim, decoder_embed_dim = args.decoder_embed_dim,
                                decoder_depth=args.trans_deep_decoder, decoder_num_heads=args.num_heads, pos_weight = args.pos_weight)
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        for data_iter_step, (samples, pos, info, h) in enumerate(data_loader_train):
            ...
            poly_reserve, pos_reserve, h_reserve, pos_tar = random_masking(samples, pos, info, h, remain_num)
            ...
            if args.add_maxpool:
                loss, _, loss_bce = model(poly_reserve, pos_reserve, h_reserve, pos_tar)
            else:

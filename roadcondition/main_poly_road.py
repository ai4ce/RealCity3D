...
def get_args_parser():
    ...
    parser.add_argument('--datainfo_path', default='../results/states/polyinfo_np.npy', type=str,
                        help='dataset path')
    parser.add_argument('--dataroad_path', default='../results/states/polyroad_np.npy', type=str,
                        help='dataset path')
...
def random_masking(x, pos, info, remain_num, max_build = 250):
        """
...
def main(args):
    ...
    dataset_train = PolyDataset(args.data_path, args.datapos_path, args.datainfo_path, args.dataroad_path, train=True,split_ratio = args.split_ratio)

    dataset_valid = PolyDataset(args.data_path, args.datapos_path, args.datainfo_path, args.dataroad_path, train=False,split_ratio = args.split_ratio)
    ...
    model = MAGECityPolyGen(drop_ratio = args.drop_ratio, num_heads=args.num_heads, device = args.device, max_build=args.max_build,
                        depth=args.trans_deep, embed_dim=args.embed_dim, decoder_embed_dim = args.decoder_embed_dim, discre=args.discre,
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        for data_iter_step, (samples, pos, info, road) in enumerate(data_loader_train):

            if data_iter_step % args.accum_iter == 0: 
                lr_sched.adjust_learning_rate(optimizer, data_iter_step / len(data_loader_train) + epoch, args)

            # remain_num = min(int(torch.randint(0, 30, (1,))), int(torch.min(info[:, 0])))
            poly_reserve, pos_reserve, poly_tar, pos_tar, len_tar = random_masking(samples, pos, info, remain_num)
            ...
            loss_l1, loss_len, _ = model(poly_reserve, pos_reserve, pos_tar, poly_tar, len_tar, road)

            loss = loss_l1 + args.loss_weight*loss_len

...
class MAGECityPolyGen(nn.Module):
    ...
    def __init__(self, max_poly=20, embed_dim=256, depth=12, num_heads=8,  max_bulid = 60, 
                 ...
                 mlp_ratio=4., drop_ratio = 0.1,  discre = 50, device = 'cuda', quant = 100,
    ...
    def add_random_noise(self, tensor):
        ...
        noisy_tensor = torch.clamp(tensor + noise, 0, 49)
        return noisy_tensor
    ...
    def forward(self, poly, pos, postar, polytar, len_tar, noise = False):
        ...
        if noise:
            postar = self.add_random_noise(postar)
    ...
    def infgen(self, poly, pos, postar):
        ...
        for k in range(latentin.shape[0]): 
            ...
            inputpoly = torch.argmax(out[0,-1,:2*self.quant].reshape(2,self.quant), dim = -1)*(500/self.quant).unsqueeze(0).unsqueeze(0)

            for i in range(19):
                out = self.automodel(latentin_, inputpoly)
                if torch.sigmoid(out[0, -1, 2*self.quant])>0.5:
                        # print('end:', torch.sigmoid(out[0, -1, 2]))
                    break
                cat_new = torch.argmax(out[0,-1,:2*self.quant].reshape(2,self.quant), dim = -1)*(500/self.quant)
                inputpoly = torch.cat([inputpoly, cat_new.unsqueeze(0).unsqueeze(0)], dim = 1)
        ...
        for i in range(latentautoin.shape[0]):
            ...
            for i in range(19):
                ...
                if torch.sigmoid(out[0, -1, 2*self.quant])>0.5:
                    # print('end:', torch.sigmoid(out[0, -1, 2]))
                    break
                cat_new = torch.argmax(out[0,-1,:2*self.quant].reshape(2,self.quant))*(500/self.quant)
                inputpoly = torch.cat([inputpoly, cat_new.unsqueeze(0).unsqueeze(0)], dim = 1)
                point.append(np.array(out[0, -1, :2].cpu()))
                step += 1
            # print('step:', step)

...
def main(args):
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        for data_iter_step, (samples, pos, info) in enumerate(data_loader_train):
            ...
            poly_reserve, pos_reserve, poly_tar, pos_tar, len_tar = random_masking(samples, pos, info, remain_num, max_build=args.max_build)

            poly_reserve = poly_reserve.to(device)
            pos_reserve = pos_reserve.to(device)
            pos_tar = pos_tar.to(device)
            poly_tar = poly_tar.to(device)
            loss_l1, loss_len, _ = model(poly_reserve, pos_reserve, pos_tar, poly_tar, len_tar)

...
class MAGECityPosition(nn.Module):
    ...
    def forward(self, poly, pos, postar, ablation = False, generate = False):
...
class MAGECityPosition_Maxpool(nn.Module):
    ...
    def forward(self, poly, pos, postar):
        bsz, _, _, _ = poly.shape
...
class MAGECityPosition_zeroinput(nn.Module):
    """ Masked Autoencoder with VisionTransformer backbone
    """
    def __init__(self, embed_dim=256, depth=12, num_heads=8,  patch_size = 5, patch_num = 10, 
                 decoder_embed_dim=64, decoder_depth=4, decoder_num_heads=8, pos_weight = 100,
    ...
    def forward(self, poly, pos, postar, generate = False):
        bsz, _, _, _ = poly.shape
...
#         self.avgpool2 = torch.nn.AvgPool2d(kernel_size=5)
...
#         pred_2 = self.avgpool1(pred)
#         tar_pos_2 = self.avgpool1(tar_pos)
#         loss_2 = self.l1loss(pred_2, tar_pos_2)

#         pred_3 = self.avgpool2(pred_2)
#         tar_pos_3 = self.avgpool2(tar_pos_2)
#         loss_3 = self.l1loss(pred_3, tar_pos_3)

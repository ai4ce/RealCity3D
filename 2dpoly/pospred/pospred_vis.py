...
def main(args):
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        for valid_step, (samples, pos, info) in enumerate(data_loader_valid):
            ...
            posimg_gt = np.zeros((50,50,3), dtype=np.uint8)

            for pos in pos_reserve[0]:
                pos = pos.detach().cpu()
                cv2.circle(posimg_gt,(int(pos[0]), int(pos[1])),0,(0,0,255))

            for pos in pos_tar[0]:
                pos = pos.detach().cpu()
                cv2.circle(posimg_gt,(int(pos[0]), int(pos[1])),0,(255,255,255))
            ...
            cv2.imwrite(f'{args.save_dir}'+str(valid_step) +'.jpg',img_pred)

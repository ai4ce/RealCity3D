...
def point_line_distance_range(lines:torch.Tensor, points:torch.Tensor):
    ...
    Return:
        torch.Tensor: (..., 24)
    """
    x1 = lines[..., 0:1, 0].type(torch.float32)       # (..., 24, 1)
    y1 = lines[..., 0:1, 1].type(torch.float32)         # (..., 24, 1)
    x2 = lines[..., 1:2, 0].type(torch.float32)         # (..., 24, 1)
    y2 = lines[..., 1:2, 1].type(torch.float32)         # (..., 24, 1)
    x = points[..., 0].type(torch.float32)              # (..., 24, 6)
    y = points[..., 1].type(torch.float32)              # (..., 24, 6)
    den = (y2-y1)*x - (x2-x1)*y + x2*y1 - y2*x1
    #print('den: ',den)
    print('den.dtype: ',den.dtype)
...
def point_line_projection_range(lines:torch.Tensor, points:torch.Tensor):
    ...
    # print('proj_: ',proj, torch.norm(vec, dim=-1, keepdim=False))

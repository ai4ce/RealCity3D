...
def sort_indices(vertices:torch.Tensor, mask:torch.Tensor):
    ...
    mean = torch.sum(vertices * (mask.float().unsqueeze(-1)), dim=2, keepdim=True) / (num_valid.unsqueeze(-1).unsqueeze(-1)+EPSILON)
    ...
    #print('vertices_normalized: ',vertices_normalized)

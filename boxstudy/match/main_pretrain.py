...
from models_mae_patch import MaskedAutoencoderViT
...
def get_args_parser():
    ...
    parser.add_argument('--output_dir', default='../../results/train/box/patch/output_dir_patch',
                        help='path where to save, empty for no saving')
    parser.add_argument('--log_dir', default='../../results/train/box/patch/output_log_patch',
...
def main(args):
    ...
    #print(dataset_train)
    if args.do_valid == True:
        dataset_valid = ValidDataset(args.data_path,train=False,split_ratio = args.split_ratio)
    else:
        dataset_valid = ValidDataset(args.data_path,train=True,split_ratio = args.split_ratio)
    ...
    model = MaskedAutoencoderViT(mask_ratio = args.mask_ratio, 
                                device = args.device, activation=args.activation, 
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        for data_iter_step, samples in enumerate(data_loader_train):
            ...
            loss, _, _, loss_giou, loss_class, loss_l1  = model(samples, mask_ratio=args.mask_ratio)
        ...
        for valid_step, valid_samples in enumerate(data_loader_valid):
            ...
            with torch.no_grad():
                ...
                loss, _, _, loss_giou, loss_class, loss_l1  = model(valid_samples, mask_ratio=args.mask_ratio)
